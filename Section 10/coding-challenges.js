 ...
 * Take all the time you need. Think abut when exactly the callback
 * function is executed, and what that means for the variables involved
 * in this example.
 */

/**
 * Why did this work? How does the callback function in the event listener
 * get access to the `header` variable?
 *
 * It is because of closure. By the time the callback in the event
 * listener is executed, the IIFE is long gone i.e. it has executed what
 * it needed to execute and it's execution context has been removed from
 * the call stack. Along with it, the `header` variable is gone as well.
 *
 * So, even though the environment in which the event listener callback
 * function was created, is already gone, it is still able to access the
 * variables that were created in the variable environment, in which the
